    ...
    local OverrideLFD = function(self, override)
		...
		if  LFDRole then
			...
			elseif role == 'HEALER' then
				LFDRole:SetTexture[[Interface\AddOns\AnyLua60\iipUI\art\group\role\healer.tga]]
				LFDRole:SetTexCoord(0, 1, 0, 1)
				LFDRole:Show()
			elseif role == 'TANK' then
				LFDRole:SetTexture[[Interface\AddOns\AnyLua60\iipUI\art\group\role\tank.tga]]
				LFDRole:SetTexCoord(0, 1, 0, 1)
				LFDRole:Show()
			else
				LFDRole:SetTexture''
	...
	local UpdateModifier = function(self, unit)
		if  self.Modifier then
			...
			if  UnitIsDead(unit) then
				self.Modifier:SetTexture[[Interface\AddOns\AnyLua60\iipUI\art\group\modifier\dead]]
				self.Modifier:Show()
                UpdateName(self, unit, false)
                if self.Health.percent then self.Health.percent:Hide() end
			elseif UnitIsGhost(unit) then
				self.Modifier:SetTexture[[Interface\AddOns\AnyLua60\iipUI\art\group\modifier\ghost]]
				self.Modifier:Show()
                UpdateName(self, unit, false)
                if self.Health.percent then self.Health.percent:Hide() end
			elseif UnitIsAFK(unit) then
				self.Modifier:SetTexture[[Interface\AddOns\AnyLua60\iipUI\art\group\modifier\afk]]
				self.Modifier:Show()
                UpdateName(self, unit, false)
                if self.Health.percent then self.Health.percent:Hide() end
            end
